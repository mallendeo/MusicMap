...
Youtify.prototype.guessYouTubeMusicVideo = (description) => {
  return description.match(/soundcloud|spotify|itunes|bandcamp/igm);
};

Youtify.prototype.isMix = function (description) {
  let regex = /track\s?list/igm;
  return description.match(regex) ? true : false;
};

Youtify.prototype.getSpotifyUrlFromDescription = (description) => {
...
Youtify.prototype.getInfoFromTitle = (videoTitle) => {
...
Youtify.prototype.getToken = (token, refresh) => {
  return new Promise((resolve, reject) => {
    let req = request
